...
import type { PluginRule, PluginRuleContext, PluginRuleViolation } from '../types.js';

interface EnforceSharedImportsOptions {
  alias?: string;
}

interface ReplacementHint {
  original: string;
  replacement: string;
  quote: string;
}
...
interface AnalyzedIssue {
  violation: PluginRuleViolation;
  replacements: ReplacementHint[];
}
...
function computeLocation(source: string, index: number): { line: number; column: number } {
...
function analyzeFile(source: string, file: string, alias: string): AnalyzedIssue | null {
  const replacements: ReplacementHint[] = [];
  ...
  const violation: PluginRuleViolation = {
...
function createEnforceSharedImportsRule(
  options: EnforceSharedImportsOptions = {},
): PluginRule {
  const alias = normalizeAlias(options.alias ?? DEFAULT_ALIAS);

  async function scanProject(
    context: PluginRuleContext,
  ): Promise<Array<{ issue: AnalyzedIssue; absolutePath: string; source: string }>> {
    const findings: Array<{ issue: AnalyzedIssue; absolutePath: string; source: string }> = [];
    ...
    async fix(context) {
      ...
      for (const { issue, absolutePath, source } of findings) {
        ...
        if (updated !== source) {
          ...
          if (context.verbose) {
            console.log(
              `[arch-lens] Replaced shared import in ${issue.violation.file} -> ${alias}*`,
            );
          }
...
export {
  enforceSharedImportsPlugin,
  enforceSharedImportsRule,
  createEnforceSharedImportsRule,
};

...
4. **플러그인 추가**: 내장 규칙으로 부족한 정책은 `@arch-lens/plugins`로 직접 작성해 `plugins`+`rules`로 활성화합니다.
...
- 플러그인은 `@arch-lens/plugins`의 `definePlugin()`으로 규칙 묶음을 export하고, config의 `plugins` 배열(또는 CLI `--plugin`)에 등록합니다. bare 패키지 지정자(`@scope/rules`)도 지원합니다.
